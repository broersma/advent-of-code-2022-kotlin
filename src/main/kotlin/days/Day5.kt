...
class Day5 : Day(5) {
    ...
    override fun partOne(): String {
        ...
        val stacks = input[0]
            ...
            .drop(1)
            .map {
                it.drop(1).filterIndexed { i, _ -> i % 4 == 0}.toList()
            }
            .fold(Array(10) { ArrayDeque<Char>() }) { acc, row ->
                acc.apply { row.withIndex().filter { it.value.isLetter() }.onEach { acc[it.index].add(it.value) }}
             }
    ...
    override fun partTwo(): String {
        ...
        proc.forEach {
            ...
            stacks[to].addAll(sequence {
                repeat ( num ) {
                    yield(stacks[from].removeLast())
                }
            }.toList().reversed())
        }
